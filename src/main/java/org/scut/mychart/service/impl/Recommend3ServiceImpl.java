...
import org.scut.mychart.mapper.Recommend3Mapper;
 ...
 * @author gzw
 */
@Service("recommend3Service")

public class Recommend3ServiceImpl {
    @Resource  
    private Recommend3Mapper recommendDao;
...
//    	List<RecommendResultModel> resultModels  =new ArrayList<>();
		List<Recommend2ResultModel> rating = recommendDao.selectRecommend3Rating(param);
		if (rating.size()==0) {
			...
			if (userid.size() == 0) {
				...
				if (minid == -1) {
					...
					rating = recommendDao.selectRecommend3Rating(param);
